...
function SortingViz({ data }: { data: SortingVisualization }) {
  ...
  const getColor = (index: number): string => {
    if (data.sorted?.includes(index)) return '#00FF00';
    if (data.swapping?.includes(index)) return '#FF0080';
    if (data.comparing?.includes(index)) return '#eab308';
    if (data.pivot === index) return '#00CC00';
    return '#2a2a2a';
  };
  ...
  return (
    <div className="flex items-end justify-center gap-1 h-40 px-4">
      {data.array.map((val, i) => (
        ...
        >
          <span className="text-xs text-gray-600 mb-1 font-mono">{val}</span>
          <motion.div
...
function ArrayViz({ data }: { data: ArrayVisualization }) {
  return (
    <div className="flex flex-col items-center gap-2">
            ...
            <motion.div
              ...
              className={`w-10 h-10 flex items-center justify-center border font-mono text-sm font-bold transition-colors ${
                isFound
                  ...
                  : 'border-[#2a2a2a] bg-[#161616] text-gray-500'
              }`}
      ...
      <div className="flex gap-1">
        {data.array.map((_, i) => (
          <div key={i} className="w-10 text-center text-xs text-gray-700 font-mono">
            {i}
          </div>
        ))}
      </div>
          ...
          <g key={i}>
            ...
            {edge.weight !== undefined && (
              <text x={(fromX + toX) / 2} y={(fromY + toY) / 2 - 4}
                fill="#00FF00" fontSize="9" textAnchor="middle" fontFamily="monospace">
                {edge.weight}
              </text>
            )}
          </g>
...
export default function Visualization({ data }: VisualizationProps) {
  return (
    <div className="border border-[#2a2a2a] bg-[#161616] p-4">
      <div className="text-xs font-mono text-gray-600 mb-3 tracking-widest">{'// VISUALIZATION'}</div>
