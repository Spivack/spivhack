import { Code2, Menu, X } from 'lucide-react';
import { Link } from 'react-router-dom';

interface HeaderProps {
  sidebarOpen: boolean;
  onToggleSidebar: () => void;
}

export default function Header({ sidebarOpen, onToggleSidebar }: HeaderProps) {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 h-14 bg-[#060e06] border-b border-green-900/80">
      <div className="flex items-center gap-3">
        <button
          onClick={onToggleSidebar}
          className="p-1.5 text-green-700 hover:text-green-400 hover:bg-green-950 transition-colors lg:hidden"
          aria-label="Toggle sidebar"
        >
          {sidebarOpen ? <X size={20} /> : <Menu size={20} />}
        </button>
        <Link to="/topics" className="flex items-center gap-2 cursor-pointer">
          <div className="p-1.5 border border-green-800 bg-green-950">
            <Code2 size={18} className="text-green-400" />
          </div>
          <span className="font-pixel text-2xl tracking-wide leading-none">
            <span className="text-green-100">R00t</span>
            <span className="text-gradient">Stack</span>
          </span>
        </Link>
        <span className="hidden sm:block text-green-800 text-xs font-mono border-l border-green-900 pl-3">
          DSA_REF v3.8.17
        </span>
      </div>

      <div className="flex items-center gap-3">
        <a
          href="https://github.com/Spivack/r00tstack"
          target="_blank"
          rel="noopener noreferrer"
          className="p-1.5 text-green-700 hover:text-green-400 hover:bg-green-950 transition-colors"
          aria-label="GitHub repository"
        >
          <svg viewBox="0 0 24 24" width={18} height={18} fill="currentColor" aria-hidden="true">
            <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.02 10.02 0 0 0 22 12.017C22 6.484 17.522 2 12 2z" />
          </svg>
        </a>
      </div>
    </header>
  );
}
